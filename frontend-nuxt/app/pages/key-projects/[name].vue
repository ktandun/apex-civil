<template>
  <div v-if="keyProject" class="key-project-details">
    <DefaultBreadcrumb
      parent="KEY PROJECTS"
      parentLink="key-projects"
      :title="keyProject.title"
    ></DefaultBreadcrumb>
    ...
    <div class="content">
      ...
      <div class="points">
        ...
        <div>
          ...
          <div
            class="point-details"
            v-for="service in keyProject.service"
            :key="service"
          >
            {{ service }}
          </div>
      ...
      <div class="cards">
        <DefaultCard
          ...
          v-bind="otherProject"
          :link="{
            name: 'key-projects-name',
            params: { name: otherProject.html },
          }"
        >
        </DefaultCard>
      </div>
...
const setup = () => {
  keyProject.value = keyProjects.find(
    (project) => project.html == route.params.name
  );
  otherProjects.value = keyProjects.filter(
    (project) => project.html != route.params.name
  );

  useSeoMeta({
    title: `Apex Civil - ${keyProject.value?.title}`,
  });
};
