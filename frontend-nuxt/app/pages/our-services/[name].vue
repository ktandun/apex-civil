              ...
              <div>
                <router-link
                  :to="{
                    ...
                    params: { name: otherService.html },
                  }"
                  v-slot="{ href, route, navigate }"
                >
                  <button :href="href" @click="navigate" class="primary">
                    View
                  </button>
                </router-link>
              </div>
...
const ourServiceName = route.params.name;

let ourService = ref<OurService | undefined>();
let otherOurServices = ref<OurService[] | undefined>();

const setup = () => {
  ourService.value = ourServices.find(
    (service) => service.html == ourServiceName
  );
  otherOurServices.value = ourServices.filter(
    (service) => service.html != ourServiceName
  );

  useSeoMeta({
    title: `Apex Civil - ${ourService.value?.title}`,
  });
};
